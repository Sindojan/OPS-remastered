...
/** Reconnect delay after a normal stream end (server closed connection cleanly). */
const RECONNECT_NORMAL_MS = 3000;
/** Max reconnect delay for error backoff. */
const RECONNECT_MAX_MS = 30000;

/** Max log entries per agent before oldest are dropped. */
  ...
  const connect = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/agent-activity/stream`,
        {
      ...
      // SSE spec: accumulate event type and data fields, dispatch on blank line
      let eventType = "";
      ...
      while (!abortRef.current) {
        ...
        for (const rawLine of lines) {
          const line = rawLine.trimEnd(); // only trim trailing whitespace (CR etc.)
      ...
      // Connection error (network failure, abort, etc.)
    } finally {
      if (!abortRef.current) {
        setConnected(false);
        // Normal stream end: reconnect quickly. Error: exponential backoff.
      ...
      try {
        if (type === "snapshot" || type === "message") {
          ...
          setRealtimeEvents((prev) => {
            ...
            // Handle delegation events for target agent
          ...
          // Accumulate into activity log (per agent)
          setActivityLog((prev) => {
            const next = new Map(prev);
            ...
            next.set(agentId, updated.length > MAX_LOG_PER_AGENT
              ...
              : updated);
            // Also log delegation events under the target agent
